...
abstract class RegisterFile[T <: Data](
  ...
  (implicit p: Parameters) extends BoomModule // 继承BOOM基础模块
{
  /**
   * 模块IO端口定义
   * 采用BoomBundle统一封装信号
  ...
  val io = IO(new BoomBundle {
    ...
    val arb_read_reqs = Vec(numReadPorts, Flipped(Decoupled(
    ...
    val write_ports = Vec(numWritePorts, Flipped(Valid(new Bundle {
      val addr = Output(UInt(maxPregSz.W)) // 最大物理寄存器地址位宽
    ...
    extends RegisterFile(dType, numRegisters, numReadPorts, numWritePorts)
{
  ...
  override def toString: String = BoomCoreStringPrefix(
    "==" + typeStr + " Regfile==",       // 分隔线
    "Num RF Read Ports     : " + numReadPorts,    // 显示读端口数
    "Num RF Write Ports    : " + numWritePorts,   // 显示写端口数 
    "RF Cost (R+W)*(R+2W)  : " + rf_cost          // 显示硬件成本
  )
