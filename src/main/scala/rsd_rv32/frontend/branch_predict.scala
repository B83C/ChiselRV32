...
class BP_IO (implicit p: Parameters) extends CustomBundle {
  ...
  val instr_mask = Vec(p.CORE_WIDTH, Bool())
...
class BranchPredictor(implicit p: Parameters) extends Module {
  ...
  io.instr_mask := 0.U(p.CORE_WIDTH.W).asBools
  when(btb_hit) {
    ...
    io.instr_mask := (~(btb_hit_oh | (btb_hit_oh - 1.U))).asBools
  }
  ...
  when(io.rob_commitsignal.valid) {
    commit_signal.foreach{cs => {
      when(cs.valid) {
        ...
        // TODO
        btb_entry.isConditional := cs.rob_type === ROBType.Branch
    ...
    val branch_cnt_til_taken = PopCount(commit_signal.map(x => x.valid && x.rob_type === ROBType.Branch))
    // TODO
