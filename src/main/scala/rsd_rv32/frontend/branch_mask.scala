...
class BranchMask(implicit p: Parameters) extends CustomModule {
    ...
    // to everyone else
    io.bu_update := DontCare
    io.bu_commit := DontCare
    
    // from IF
    io.bu_mask_freelist_deq := DontCare
    io.bu_mask_freelist_full := DontCare
    
    // to IF
    io.current_branch_mask_with_freed := DontCare
    io.branch_freed := DontCare
