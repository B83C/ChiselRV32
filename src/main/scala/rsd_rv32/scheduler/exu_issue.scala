...
class exu_issue_IO(implicit p: Parameters) extends CustomBundle {
    ...
    //val dst_FU = (Vec(p.CORE_WIDTH, UInt(log2Ceil(p.ALU_NUM).W)))  //发射的指令的目标功能单元
    //val issue_uop = Valid(Vec(p.CORE_WIDTH, new EXUISSUE_EXU_uop()))  //发射的指令(包含操作数的值)
    // val value_o1 = (Vec(p.CORE_WIDTH, UInt(p.XLEN.W))) //发射的指令的操作数1
    // val value_o2 = (Vec(p.CORE_WIDTH, UInt(p.XLEN.W))) //发射的指令的操作数2

    //PRF
    val prf_raddr1 = (Vec(p.CORE_WIDTH, UInt(log2Ceil(p.PRF_DEPTH).W))) //PRF读地址1
    val prf_raddr2 = (Vec(p.CORE_WIDTH, UInt(log2Ceil(p.PRF_DEPTH).W))) //PRF读地址2
...
//    val wb_uop2 = Flipped((Vec(p.FU_NUM - p.BU_NUM - p.STU_NUM, Valid(new ALU_WB_uop()))))  //来自alu、mul、div、load pipeline的uop
    ...
    //输出至Dispatch Unit的信号
    val exu_issued_index = (Vec(p.CORE_WIDTH, Valid(UInt(log2Ceil(p.EXUISSUE_DEPTH).W)))) //更新IQ Freelist

    //with ROB
    val rob_commitsignal = Flipped(Vec(p.CORE_WIDTH, Valid(new ROBContent()))) //ROB提交时的广播信号，发生误预测时对本模块进行冲刷

    //以下是测试时需要的输出
    val queue = (Vec(p.EXUISSUE_DEPTH, new exu_issue_content())) //发射队列的内容
}
...
class exu_iq_select_logic(implicit p: Parameters) extends CustomModule {
    val io = IO(new Bundle {
        ...
        val sel_index = (Vec(2, Valid(UInt(log2Ceil(p.EXUISSUE_DEPTH).W)))) //选择的指令的索引
    })
    //生成ready序列
