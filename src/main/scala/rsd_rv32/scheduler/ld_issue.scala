    ...
    withReset(reset.asBool) {
        dis_uop.foreach { uop =>
            when(VALID && uop.valid && !should_flush)  {
                ...
                issue_queue(uop.bits.iq_index).st_busy := VecInit(((uop.bits.stq_mask | io.st_issue_busy_snapshot.asUInt) & Mux(io.st_issued_index.valid, ~(1.U << io.st_issued_index.bits)(p.STISSUE_DEPTH - 1, 0), 0.U)).asBools)
