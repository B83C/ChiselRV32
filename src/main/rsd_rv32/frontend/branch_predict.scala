...
class BP_IFU_Interface(val fetch_width : Int , val GHR_width : Int) extends Bundle {
    val PC_cur = Input(UInt(32.W)) //当前IFU的PC值
    val PC_target = Output(UInt(32.W)) //预测的下个cycle取指的目标地址
    val BTB_Hit = Output(Vec(fetch_width, Bool())) //1代表hit，0相反；将最年轻的命中BTB的置为1，其余为0
    val BHT_Taken = Output(Bool()) //branch指令的BHT的预测结果；1代表跳转，0相反
    val GHR = Output(UInt(GHR_width.W)) //作出预测时的全局历史寄存器快照
}
