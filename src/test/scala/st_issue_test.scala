...
class st_issue_test extends AnyFlatSpec with ChiselScalatestTester {
  ...
  "st_issue" should "accept the instrs from dispatch unit and issue correctly" in {
    test(new st_issue_queue()) { dut =>
      ...
      dut.io.st_issue_uop(0).bits.iq_index.poke(0.U)
      dut.io.st_issue_uop(1).valid.poke(0.U)
      dut.clock.step()
      dut.io.st_issue_uop(0).valid.poke(1.U)
      dut.io.st_issue_uop(0).bits.iq_index.poke(2.U)
      dut.io.st_issue_uop(1).valid.poke(1.U)
      dut.io.st_issue_uop(1).bits.iq_index.poke(1.U)
      dut.io.st_issue_uop(1).bits.ps1.poke(2.U)
      dut.io.st_issue_uop(1).bits.ps2.poke(3.U)
      dut.clock.step(2)
  ...
  "st_issue" should "accept 2 instrs from dispatch unit and issue correctly" in {
    test(new st_issue_queue()) { dut =>
      ...
      dut.io.st_issue_uop(0).valid.poke(1.U)
      dut.io.st_issue_uop(1).valid.poke(1.U)
      dut.io.st_issue_uop(0).bits.iq_index.poke(5.U)
      dut.io.st_issue_uop(1).bits.iq_index.poke(6.U)
      dut.io.st_issue_uop(0).bits.ps1.poke(2.U)
      dut.clock.step()
      ...
      dut.io.st_issue_uop(0).valid.poke(1.U)
      dut.io.st_issue_uop(0).bits.iq_index.poke(4.U)
      dut.clock.step(3)
      println("test pass")
    }
  }
  ...
  "st_issue" should "flush correctly when mispred" in {
    test(new st_issue_queue()) { dut =>
      dut.io.st_issue_uop(0).valid.poke(1.U)
      dut.io.st_issue_uop(0).bits.iq_index.poke(3.U)
      dut.io.st_issue_uop(1).valid.poke(1.U)
      dut.io.st_issue_uop(1).bits.iq_index.poke(4.U)
      dut.clock.step()
      dut.io.st_issue_uop(0).bits.iq_index.poke(2.U)
      dut.io.st_issue_uop(1).bits.iq_index.poke(1.U)
      dut.clock.step()
