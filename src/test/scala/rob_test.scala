...
class rob_test extends AnyFlatSpec with ChiselScalatestTester {
  ...
  "rob" should "correct" in {
    test(new ROB()) { c =>
      c.io.rob_uop(0).valid.poke(1.U)
      c.io.rob_uop(0).bits.instr_addr.poke("h0000ffff".U)
      c.io.rob_uop(0).bits.instr_type.poke(InstrType.ALU)
      c.io.rob_uop(0).bits.pdst.poke(2.U)
      c.io.rob_uop(0).bits.rd.poke(1.U)
      c.clock.step()
      c.io.rob_uop(0).valid.poke(0.U)
      c.clock.step()
      println("pass!")
    }
  }
  "rob" should "receive 2 uop correctly" in {
    test(new ROB()) { c =>
      ...
      c.io.rob_uop(1).valid.poke(1.U)
      c.io.rob_uop(1).bits.instr_addr.poke("h0000abcd".U)
      c.io.rob_uop(1).bits.instr_type.poke(InstrType.Branch)
      c.io.rob_uop(1).bits.btb_hit.poke(BTBHit.H)
      c.io.rob_uop(1).bits.GHR.poke(2.U)
      c.clock.step()
      c.io.rob_uop(0).valid.poke(1.U)
      c.io.rob_uop(1).valid.poke(0.U)

      c.io.rob_uop(0).bits.instr_addr.poke("h00001234".U)
      c.io.rob_uop(0).bits.instr_type.poke(InstrType.ST)
      c.clock.step()
      c.io.rob_uop(0).valid.poke(0.U)
      c.clock.step()
      println("pass!")
  ...
  "rob" should "flush correctly" in {
    test(new ROB()) { c =>
      c.io.rob_uop(0).valid.poke(1.U)
      c.io.rob_uop(0).bits.instr_addr.poke("h0000abcd".U)
      c.io.rob_uop(0).bits.instr_type.poke(InstrType.Branch)
      c.io.rob_uop(0).bits.btb_hit.poke(BTBHit.H)
      c.clock.step()
      c.io.rob_uop(0).bits.instr_addr.poke("h0000aaaa".U)
      c.io.rob_uop(0).bits.instr_type.poke(InstrType.DIV_REM)
      c.io.rob_uop(0).bits.pdst.poke(2.U)
      c.clock.step()
      c.io.rob_uop(0).valid.poke(0.U)
      c.io.bu_wb_uop(0).valid.poke(1.U)
  ...
  "rob" should "head and tail move correctly" in {
    test(new ROB()) { c =>
      c.io.rob_uop(0).valid.poke(1.U)
      c.io.rob_uop(0).bits.instr_type.poke(InstrType.ALU)
      c.io.rob_uop(0).bits.instr_addr.poke(1.U)
      c.io.rob_uop(1).valid.poke(1.U)
      c.io.rob_uop(1).bits.instr_type.poke(InstrType.ALU)
      c.io.rob_uop(1).bits.instr_addr.poke(1.U)
      c.clock.step(5)
